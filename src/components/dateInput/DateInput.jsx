...
const DateInput = ({
  ...
  const isOutsideDate = useCallback(
    (date) => {
      return (
        date.getFullYear() !== shownMonthObj.getFullYear() ||
        date.getMonth() !== shownMonthObj.getMonth()
      );
    },
    [shownMonthObj]
  );
  ...
  const handleDateClick = useCallback(
    (date) => {
      ...
      switch (mode) {
        ...
        case "multiple": {
          const isASelectedDate = selected.some(
            (dt) => dt.getTime() === date.getTime()
          );
          if (isASelectedDate) {
            setSelected(
              selected.filter((dt) => dt.getTime() !== date.getTime())
            );
        ...
        case "range": {
          if (!selected.from && !selected.to) {
            ...
            selected.from.getTime() < date.getTime() &&
            date.getTime() < selected.to.getTime()
          ) {
            setSelected({ from: date, to: date });
          } else if (
            selected.from.getTime() === date.getTime() ||
            selected.to.getTime() === date.getTime()
          ) {
            setSelected({ from: date, to: date });
          } else if (
            !selected.from ||
  ...
  useEffect(() => {
    ...
    switch (mode) {
      ...
      case "range": {
        ...
        setSelected({
          from: getOnlyDateFromDate(value?.from),
          to: getOnlyDateFromDate(value?.to),
        });
        break;
      }
      default:
  ...
  useEffect(() => {
    if (monthYearSelectMode) {
      setMonthYearSelection({
        month: shownMonthObj.getMonth(),
        year: shownMonthObj.getFullYear(),
      });
    }
  ...
  useEffect(() => {
    ...
    const checkKey = (e) => {
      ...
      if (e.key === "Home") {
        ...
        while (
          isFullyDisabledDate(dt) &&
          dt.getTime() <= tabbedDate.getTime()
        ) {
          dt = new Date(dt.getFullYear(), dt.getMonth(), dt.getDate() + 1);
        }
        setTabbedDate(dt);
  ...
  useEffect(() => {
    ...
    if (tabbedDate.getMonth() !== shownMonthObj.getMonth())
      setShownMonthObj(
        new Date(shownMonthObj.getFullYear(), tabbedDate.getMonth())
      );
  ...
  useEffect(() => {
    ...
    if (
      selected.every((dt, i, arr) => !i || arr[i - 1].getTime() <= dt.getTime())
    )
      return;
    setSelected(
      [...selected].sort((dt1, dt2) => dt1.getTime() - dt2.getTime())
    );
  }, [selected, mode]);
  ...
  const handlePrevMonthClick = () => {
    setTabbedDate(null);
    setShownMonthObj(
      new Date(shownMonthObj.getFullYear(), shownMonthObj.getMonth() - 1)
    );
  };
  ...
  const handleReset = () => {
    switch (mode) {
      case "single": {
        ...
        setShownMonthObj(
          getOnlyDateFromDate(value || defaultValue || new Date())
        );
        break;
      }
      case "multiple": {
        setSelected(value?.map((dt) => getOnlyDateFromDate(dt)) || []);
        setShownMonthObj(
          getOnlyDateFromDate(value?.[0] || defaultValue?.[0] || new Date())
        );
        break;
      }
      case "range": {
        ...
        setShownMonthObj(
          getOnlyDateFromDate(value?.from || defaultValue?.from || new Date())
        );
        break;
      }
      default:
  ...
  const getClasses = (date) => {
    ...
    const isToday =
      date.getTime() === getOnlyDateFromDate(new Date()).getTime();
    ...
    const isSelected = (() => {
      switch (mode) {
        ...
        case "range":
          return [selected?.from, selected?.to].some(
            (dt) => dt?.getTime() === date.getTime()
          );
        default:
          return false;
  ...
  const getShownDates = () => {
    ...
    for (let i = firstDayOfCurrShownMonth - 1; i >= 0; i--) {
      shownDates.push(
        new Date(shownMonthObj.getFullYear(), shownMonthObj.getMonth(), -i)
      );
    }
    ...
    for (let i = 1; i <= 6 - lastDayOfCurrShownMonth; i++) {
      shownDates.push(
        new Date(shownMonthObj.getFullYear(), shownMonthObj.getMonth() + 1, i)
      );
    }
    return shownDates;
  };
  ...
  return (
    <>
      <div className="flex max-h-[500px] min-h-[400px] w-[calc(40px*7+16px)] flex-col rounded-sm bg-white text-sm text-gray-800 shadow-xl">
                  ...
                  <>
                    {!selected ? (
                      ...
                      <>
                        ...
                        <div>
                          {days[selected.getDay()]}, {selected.getDate()}{" "}
                ...
                return (
                  <div className="text-base">
                    {selected.from && selected.to && (
                      <>
                        {days[selected.from.getDay()]},{" "}
                        {selected.from.getDate()}{" "}
                        {months[selected.from.getMonth()]}{" "}
                        {selected.from.getFullYear()} -{" "}
                        {days[selected.to.getDay()]}, {selected.to.getDate()}{" "}
                        {months[selected.to.getMonth()]}{" "}
        ...
        <div>
          {errorMsg && (
            <div className="mx-2 my-1 text-xs text-red-500">{errorMsg}</div>
          )}
        </div>
            ...
            <div className="bg-white px-[8px] py-1.5">
              ...
              <div className="flex flex-wrap">
                {shownDates.map((shownDate, i) => (
                  <div
                    key={i}
                    className="relative flex h-[40px] w-[40px] items-center justify-center p-0.5"
                  >
                    <div
                      className={
                        ...
                        `${
                          isCurrentlyDisabledDate(shownDate)
                            ? "pointer-events-none"
                            : "cursor-pointer"
                        } ` +
                    ...
                    >
                      {(hideOutsideDays &&
                        shownDate.getMonth() !== shownMonthObj.getMonth()) ||
                      isHidden?.(shownDate)
                        ? ""
                        : shownDate.getDate()}
                    </div>
